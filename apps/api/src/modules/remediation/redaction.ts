...
const SEED_PHRASE = /\b(?:[a-z]{3,}\s+){11,23}[a-z]{3,}\b/giu;
const RPC_URL = /\bhttps?:\/\/[^\s"'<>]*(?:alchemy|infura|quicknode|rpc)[^\s"'<>]*/giu;
const MONITORING_RPC_VALUE = /MONITORING_RPC_URL\s*=\s*[^\s"'<>]+/giu;
...
export function redactSecrets(value: string): string {
  return value
    ...
    .replace(MONITORING_RPC_VALUE, "MONITORING_RPC_URL=[REDACTED_RPC_URL]")
    .replace(RPC_URL, "[REDACTED_RPC_URL]")
